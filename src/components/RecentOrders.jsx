  ...
  return (
    <div className="bg-white rounded-lg overflow-hidden">
      ...
      <div className="flex overflow-x-auto p-3 space-x-3">
        {orders.map(order => (
          <div key={order.id} className="min-w-[280px] bg-agri-gray rounded-lg relative"> {/* Added relative positioning */}

            {/* Red Stripe - Conditional Rendering */}
            {(order.status === "New" || order.status === "Pending") && (
              <div className="absolute top-0 left-0 w-full h-8 bg-red-600 flex items-center justify-between px-3">
            ...
            <div className="p-3" style={{ paddingTop: (order.status === "New" || order.status === "Pending") ? '32px' : '16px' }}>  {/*Adjusted padding*/}
